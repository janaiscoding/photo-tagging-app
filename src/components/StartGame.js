...
const StartGame = ({ targets, startGame, handleTimer}) => {
  const targetsList = targets.map((target) => {
    return (
      <motion.div
        key={target.id}
        className="bg-blue p-3 flex flex-col rounded max-h-fit items-center"
        whileHover={{ scale: 1.02 }}
        layoutId={target.id}
      >
        <a href={target.wikiLink} target="_blank" rel="noreferrer">
          <img src={target.img} alt={target.name} className="max-h-[90px] md:max-h-[150px]" />
        </a>
  ...
  return (
    <>
      <div className="min-h-[90vh] bg-pink p-3 flex items-center justify-center">
        <motion.div
          className="flex flex-col justify-center bg-soft-pink border border-solid border-grey p-3 md:p-10 rounded drop-shadow-md gap-5 items-center"
          initial={{ opacity: 0, scale: 0.7 }}
          animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.5 }}
        >
          <p className="text-center text-xl md:text-3xl">Scroll vertically and horizontally in the image.</p>
